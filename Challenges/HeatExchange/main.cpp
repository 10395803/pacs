...
int main(int argc, char** argv)
{
  ...
  const int &	outMode = param.outMode; // How to print the results
  const std::string & file = param.file; // Output file name
 ...
 // Analitic solution
    ...
    vector<double> thetaa(M+1);
     ...
     if (outMode >= 1) {
     	...
     	for(int m = 0; m<= M; m++)
        	cout << coor[m] << "\t" << sol[m] << "\t" << exact[m] << endl;
     }
